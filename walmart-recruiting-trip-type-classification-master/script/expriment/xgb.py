...
import pickle,re, os
import script.expriment.goldenFeature
import operator
...
def train_with_val():
    if os.path.isfile('dataset.pkl'):
        with open('dataset.pkl', 'rb') as fp:
            newtrain = pickle.load(fp)
            newtest = pickle.load(fp)
            newval = pickle.load(fp)
        ...
        test['tfnl1'] = test['FinelineNumber'].apply(lambda x : str(int(x) %100 + 1) )#FinelineNumber
        ...
        ############################################################
        train['DDWeek'] = train['DepartmentDescription'].map(str) + train['Weekday'].map(str) #DepartmentDescription
        ...
        train = train.drop(['DDWeek', 'DDFln1', 'DDFln2', 'DDScan', 'DDUpc'], axis = 1)#删除行
        ...
        newtest  = pd.concat([newtest,  test_log], axis =1 )#合并的两个表就睡根据列字段对齐，然后合并。最后再重新整理一个新的index。
        ...
        with open('dataset.pkl', 'wb') as fp:
            pickle.dump(newtrain, fp, -1)
            pickle.dump(newtest,  fp, -1)
            pickle.dump(newval, fp, -1)
            pickle.dump(train_Y,  fp, -1)
            pickle.dump(val_Y, fp, -1)
...
def tfidf_noval():
    ...
    with open('walmart_tfidf.pkl' , 'wb') as fp:
        pickle.dump(train_tfidf, fp, -1)
        pickle.dump(test_tfidf, fp, -1)
...
def dummpyFLNPCA():
    ...
    with open('fln.pkl', 'wb') as fp:
        pickle.dump(train_fln, fp, pickle.HIGHEST_PROTOCOL)
        pickle.dump(test_fln, fp, pickle.HIGHEST_PROTOCOL)
