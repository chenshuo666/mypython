...
def classify_by_label(labelDigit):
    ...
    for i in range(len(labelDigit)):
        if labelDigit[i] not in sampleSet:
...
def extract_department(x, _dict):
    ...
    for i in range(len(x)):
        ...
        for item in x[i]:
            dptTmp = item[3]
            if dptTmp not in dptDict:
                dptDict[dptTmp] = dptCount
                dptCount = dptCount + 1
            dptTmpCount = dptDict[dptTmp]
            if dptTmpCount not in dptFeature[i]:
...
def concate_data(x, y_digit, y_vec):
    data = numpy.zeros((len(x), x[0].shape[0]))
    label_digit = numpy.zeros(len(y_digit))
    label_vec = numpy.zeros((len(y_vec), y_vec[0].shape[0]))
    i = 0
    for num in x.keys():
        data[i] = x[num]
