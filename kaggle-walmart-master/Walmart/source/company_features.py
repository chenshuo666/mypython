...
with open('../features/fulldata_company_bought.libsvm', 'w') as f:
    for k, v in enumerate(visit):
        ...
        p = np.array(pos_trip.groupby('company').agg(sum).reset_index())
...
with open('../features/fulldata_company_count.libsvm', 'w') as f:
    for k, v in enumerate(visit):
        ...
        p = np.array(trip.groupby('company').agg('count').reset_index())
        f.write('{}\n'.format(' '.join([str(-1)] + [str(i[0] + 1) + ':' + str(i[1]) for i in p])))
        if k % 1000 == 0:
            print(k)
...
with open('../features/fulldata_company_return.libsvm', 'w') as f:
    for k, v in enumerate(visit):
        ...
        p = np.array(neg_trip.groupby('company').agg(sum).reset_index())
        f.write('{}\n'.format(' '.join([str(-1)] + [str(i[0] + 1) + ':' + str(i[1]) for i in p])))
        if k % 1000 == 0:
            print(k)
