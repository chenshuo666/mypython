...
with open('../features/fulldata_fineline_bought.libsvm', 'w') as f:
    for k, v in enumerate(visit):
        ...
        p = np.array(pos_trip.groupby('FinelineNumber').agg(sum).reset_index())
...
visit = np.unique(data.VisitNumber.values)
with open('../features/fulldata_fineline_count.libsvm', 'w') as f:
    for k, v in enumerate(visit):
        trip = data[data['VisitNumber'] == v]
        trip = trip.drop('VisitNumber', axis=1)
        if trip.empty:
        ...
        p = np.array(trip.groupby('FinelineNumber').agg('count').reset_index())
        f.write('{}\n'.format(' '.join([str(-1)] + [str(i[0] + 1) + ':' + str(i[1]) for i in p])))
        if k % 1000 == 0:
            print(k)
...
visit = np.unique(data.VisitNumber.values)
with open('../features/fulldata_fineline_per_trip.libsvm', 'w') as f:
...
data = pd.read_csv('../data/full_data_enc.csv', usecols=['VisitNumber', 'ScanCount', 'FinelineNumber'])
visit = np.unique(data.VisitNumber.values)
with open('../features/fulldata_fineline_return.libsvm', 'w') as f:
    for k, v in enumerate(visit):
        trip = data[data['VisitNumber'] == v]
        trip = trip.drop('VisitNumber', axis=1)
        if trip.empty:
        ...
        p = np.array(neg_trip.groupby('FinelineNumber').agg(sum).reset_index())
        f.write('{}\n'.format(' '.join([str(-1)] + [str(i[0] + 1) + ':' + str(i[1]) for i in p])))
        if k % 1000 == 0:
            print(k)