...
print('Number of unique trip types:{}'.format(N_y))
print('Number of visits:{}'.format(N_visit))
print('Number of departments:{}'.format(N_dpt))
...
print('Misclassfied samples :{}'.format((y_test!=y_pred_lr).sum()))
from sklearn.metrics import accuracy_score
print('Training Accuracy: {}'.format(lr.score(X_train_std,y_train)))
print('Testing Accuracy:{}'.format(accuracy_score(y_test,y_pred_lr)))
...
print('Misclassfied samples :{}' .format((y_test!=y_pred_rf).sum()))
from sklearn.metrics import accuracy_score
print('Training Accuracy: {}'.format(forest.score(X_train,y_train)))
print('Testing Accuracy: {}'.format(accuracy_score(y_test,y_pred_rf)))


importances = forest.feature_importances_
indices = np.argsort(importances)[::-1]
for f in range(len(col)):
    print("{}:{}:{}:{}".format(f+1, 30, col[f],importances[indices[f]]))
...
print('Misclassfied SVM samples:{}' .format((y_test!=y_pred).sum()))
print('Tesing Accuracy: {}'.format(accuracy_score(y_test,y_pred)))
...
bag = BaggingClassifier(base_estimator = tree,n_estimators=1000,max_samples=1.0,max_features=1.0,bootstrap=True,bootstrap_features=False,
                       n_jobs=1,
                       random_state=1)
from sklearn.metrics import accuracy_score
...
print('Misclassfied samples :{}' .format((y_test!=y_pred_knn).sum()))
from sklearn.metrics import accuracy_score
print('Training Accuracy: {}'.format(knn.score(X_train_std,y_train)))
print('Testing Accuracy: {}'.format(accuracy_score(y_test,y_pred_knn)))
...
for i in np.arange(38):    
    ...
    plt.title('Trip type {}' .format(np.unique(y)[i]))
